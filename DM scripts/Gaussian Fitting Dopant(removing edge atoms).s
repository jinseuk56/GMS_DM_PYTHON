...
image GaussianFitting(image ClusterRawData, image tmp_target, number sx, number sy, number ys, number scale, number ROIsize)
{
...
for(atomcnt1=0;atomcnt1<ys;atomcnt1++)
{
	...
	{
		if(!(getpixel(ClusterRawData,1,atomcnt1)/scale<(ROIsize-1)/2 || getpixel(ClusterRawData,0,atomcnt1)/scale<(ROIsize-1)/2 || getpixel(ClusterRawData,1,atomcnt1)/scale>sy-round(ROIsize/2) || getpixel(ClusterRawData,0,atomcnt1)/scale>sx-round(ROIsize/2)))
		{
		result("Selected Position "+getpixel(ClusterRawData,0,atomcnt1)/scale+" "+getpixel(ClusterRawData,1,atomcnt1)/scale+"\n")
		FittingAtomCount += 1
		}
	}
}
result("Number of Positions: "+FittingAtomCount+"\n")
...
for(atomcnt2=0;atomcnt2<ys;atomcnt2++)
{
	...
	if(getpixel(ClusterRawData, 2, atomcnt2)!=0)
	{
		...
		if(!(getpixel(ClusterRawData,1,atomcnt2)/scale<(ROIsize-1)/2 || getpixel(ClusterRawData,0,atomcnt2)/scale<(ROIsize-1)/2 || getpixel(ClusterRawData,1,atomcnt2)/scale>sy-round(ROIsize/2) || getpixel(ClusterRawData,0,atomcnt2)/scale>sx-round(ROIsize/2)))
		{
		top = getpixel(ClusterRawData, 1, atomcnt2)/scale-(ROIsize-1)/2
		bottom = getpixel(ClusterRawData, 1, atomcnt2)/scale+round(ROIsize/2)
		left = getpixel(ClusterRawData, 0, atomcnt2)/(ROIsize-1)/2
		right = getpixel(ClusterRawData, 0, atomcnt2)/scale+round(ROIsize/2)
...
image RealTargetPosition := realimage("Real Target Position", 4, 2, FittingAtomCount)
number atomcnt3
for(atomcnt3=0;atomcnt3<FittingAtomCount;atomcnt3++)
{
	setpixel(RealTargetPosition, 0, atomcnt3, getpixel(FittingAtom, 0, atomcnt3))
	setpixel(RealTargetPosition, 1, atomcnt3, getpixel(FittingAtom, 1, atomcnt3))
}
...
number atomcnt4
for(atomcnt4=0;atomcnt4<FittingAtomCount;atomcnt4++)
{
...
image ClusterRawData := C
number xs, ys // ys -> CountDopant
image TargetImg := B
number sx, sy
...
RealTargetImg = GaussianFitting(ClusterRawData, tmp_target, sx, sy, ys, scale, ROIsize)
