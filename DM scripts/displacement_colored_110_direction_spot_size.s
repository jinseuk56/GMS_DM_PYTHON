	...
	for(j=0;j<row-2;j=j+2){
		...
		for(k=0;k<spot_size;k++){
			for(l=0;l<spot_size;l++){
				setpixel(peak_point, (round(x1)-spot_size+k), (round(y1)-spot_size+l), white)
				setpixel(peak_point, (round(x2)-spot_size+k), (round(y2)-spot_size+l), white)
				setpixel(rgb_original, (round(x1)-spot_size+k), (round(y1)-spot_size+l), white)
				setpixel(rgb_original, (round(x2)-spot_size+k), (round(y2)-spot_size+l), white)
			}
		}
		*/
		...
		for(k=0;k<(spot_size*2+1);k++){
			for(l=0;l<(spot_size*2+1);l++){
				setpixel(peak_point, (round(B_x)-spot_size+k), (round(B_y)-spot_size+l), RGB(red*degree, 0, blue*(1-degree)))
				setpixel(rgb_original, (round(B_x)-spot_size+k), (round(B_y)-spot_size+l), RGB(red*degree, 0, blue*(1-degree)))
			}
		}
	}
}
