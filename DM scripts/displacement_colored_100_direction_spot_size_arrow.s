...
// it also show us vector arrows of each B-site displacement
...
number NoYesDialog(string prompt){
	if(TwoButtonDialog(prompt, "yes", "no")) return 1
	else return 0
}
...
number A_col, A_row
if(!getnumber("the number of A-site rows", 0, A_col)) exit(0)
if(!getnumber("the number of A-site columns", 0, A_row)) exit(0)
...
image original_arrow = -original
image arrow := rgbimage("B site displacement (only arrows)", 4, xos, yos)
arrow = RGB(255, 255, 255)

number color_displacement = NoYesDialog("Do you need the displacement colormap?")
//result("\n"+color_displacement)

number spot_size
if(color_displacement){
		...
		if(color_displacement){
			...
			for(k=0;k<(spot_size*2+1);k++){
				for(l=0;l<(spot_size*2+1);l++){
					setpixel(peak_point_y, (nearest(B_x)-spot_size+k), (nearest(B_y)-spot_size+l), RGB(red*degree_y, 0, blue*(1-degree_y)))
					setpixel(rgb_original_y, (nearest(B_x)-spot_size+k), (nearest(B_y)-spot_size+l), RGB(red*degree_y, 0, blue*(1-degree_y)))
					setpixel(peak_point_x, (nearest(B_x)-spot_size+k), (nearest(B_y)-spot_size+l), RGB(red*degree_x, 0, blue*(1-degree_x)))
					setpixel(rgb_original_x, (nearest(B_x)-spot_size+k), (nearest(B_y)-spot_size+l), RGB(red*degree_x, 0, blue*(1-degree_x)))
				}
			}
		}
		
		original_arrow.createarrowannotation(nearest(B_x), nearest(B_y), nearest(mid_x)+(mid_x-B_x)*100, nearest(mid_y)+(mid_y-B_y)*100)
		arrow.createarrowannotation(nearest(B_x), nearest(B_y), nearest(mid_x)+(mid_x-B_x)*100, nearest(mid_y)+(mid_y-B_y)*100)
		//original_arrow.createarrowannotation(round(B_x), round(B_y), round(B_x)+2, round(B_y)+2)
...
showimage(original_arrow)
original_arrow.setname("B site displacement (overlapped)")
showimage(arrow)
